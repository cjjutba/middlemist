...
| 3   | Hero "Notify me" / "Learn more" CTAs on the landing page still link to `href="#"`. They are placeholder calls-to-action while the product is "Coming soon"; intentional for now.                                | `src/app/page.tsx`                                                                    | Replace with real waitlist + about routes when the marketing page lands in a later week.                                                                                               |
| 4   | `sendEmail` doesn't fail-fast if `NODE_ENV=production` and `RESEND_API_KEY` is missing — it would silently fall through to `nodemailer` and try to reach `localhost:1026` (which would only fail at send time). | `src/lib/email/send.ts`                                                               | Add a startup guard or throw inside the function when `NODE_ENV === 'production' && !RESEND_API_KEY`. Worth doing alongside the env reader's production guards in a near-term cleanup. |
| 5   | `middleware.ts` filename is deprecated in Next 16; the framework wants `proxy.ts`. Build emits a warning but still works.                                                                                       | `src/middleware.ts`                                                                   | Rename to `proxy.ts` when the migration is convenient — single-file mechanical change.                                                                                                 |
| 6   | CSP `script-src` uses `'unsafe-inline'` (and `'unsafe-eval'` in dev) instead of nonce-based per `docs/security/xss-and-sanitization.md`.                                                                        | `src/lib/security/csp.ts`                                                             | Tighten to nonces when Next 16's nonce wiring through RSC stabilizes. Currently documented as an intentional deviation in the file's docblock.                                         |

> **Closeout (2026-04-30):** Item previously at row 3 — "retroactively annotate legacy repos with explicit return types" — has been dropped. The Week 3 review pass settled the convention: new repos (`projects.repo.ts`) carry explicit return types, and the two legacy repos (`clients.repo.ts`, `users.repo.ts`) keep their inferred types. Both styles are honored as long as the file is internally consistent and the public function signature is greppable; the multi-tenancy contract does not depend on the annotation. Future repos follow the explicit-return convention.
